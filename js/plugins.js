...
! function(t, s, e, o) {
    ...
    function g(t) {
        var s, i, o = e.createElement("div"),
            n = t;
        for (s in n)
            if (i = n[s], void 0 !== o.style[i]) return o = null, [i, s];
        return [!1]
    }
    c.prototype.init = function() {
        ...
        for (t in this.responsiveSorted)
            if (t <= o) {
                e = t;
        ...
        var s = e.createElement("div"),
            i = e.createElement(this.options.stageElement);
        ...
        var i, e, o, n = t.data("vimeo-id"),
            a = t.data("youtube-id"),
            h = t.data("width") || this.options.videoWidth,
            r = t.data("height") || this.options.videoHeight,
            d = t.attr("href");
        if (n) e = "vimeo", o = n;
        else if (a) e = "youtube", o = a;
        else {
        ...
        var i, e, o = t(s.target || s.srcElement),
            n = o.closest("." + this.options.itemClass),
            a = n.data("owl-item").videoType,
            h = n.data("owl-item").videoId,
        ...
        var s, i, e, o = this.options.items,
            n = this.num.oItems - 1;
        for (this.options.stagePadding && 1 === this.options.items && (o += 1), this.num.cItems = 2 * o, e = 0; e < o; e++) {
            var a = this.dom.$oItems.eq(e).clone(!0, !0),
                h = this.dom.$oItems.eq(n - e).clone(!0, !0);
        ...
        var s, i, e, n, a, h, r;
        if (this.pos.lcCurrent === o && (this.pos.lcCurrent = 0, this.pos.current = this.pos.currentAbs = this.options.items), t || this.updateLazyPosition(), !1 !== this.state.lcDirection)
            for (s = 0; s < this.pos.lcMovedBy; s++) "right" === this.state.lcDirection && (e = this.dom.$stage.find(".owl-item").eq(0)).appendTo(this.dom.$stage), "left" === this.state.lcDirection && (e = this.dom.$stage.find(".owl-item").eq(-1)).prependTo(this.dom.$stage), e.data("owl-item").active = !1;
        ...
        var t = "ontouchstart" in s || !!navigator.msMaxTouchPoints,
            i = s.navigator.msPointerEnabled;
        ...
        var o = "touchstart" === i.type,
            n = o ? t.targetTouches[0].pageX : i.pageX || i.clientX,
        ...
        if (this.state.isTouch && !this.state.isScrolling) {
            ...
            if (this.drag.currentX = n - this.drag.startX, this.drag.currentY = a - this.drag.startY, this.drag.distance = this.drag.currentX - this.drag.offsetX, this.drag.distance < 0 ? this.state.direction = this.options.rtl ? "right" : "left" : this.drag.distance > 0 && (this.state.direction = this.options.rtl ? "left" : "right"), this.options.loop) this.op(this.drag.currentX, ">", this.pos.minValue) && "right" === this.state.direction ? this.drag.currentX -= this.pos.loop : this.op(this.drag.currentX, "<", this.pos.maxValue) && "left" === this.state.direction && (this.drag.currentX += this.pos.loop);
            else {
        ...
        if (!this.options.freeDrag)
            for (var i = 0; i < this.num.items; i++) t > this.pos.items[i] - 30 && t < this.pos.items[i] + 30 ? s = i : this.op(t, "<", this.pos.items[i]) && this.op(t, ">", this.pos.items[i + 1 || this.pos.items[i] - this.width.el]) && (s = "left" === this.state.direction ? i + 1 : i);
        ...
        var s = this.pos.stage = t,
            i = this.dom.stage.style;
        this.support3d ? (translate = "translate3d(" + s + "px,0px, 0px)", i[this.transformVendor] = translate) : this.state.isTouch ? i.left = s + "px" : this.dom.$stage.animate({
            left: s
        ...
        var e = this.pos.currentAbs,
            o = this.pos.currentAbs,
            n = this.pos.currentAbs + t,
            a = o - n < 0;
        ...
        if (!this.options.loop && !this.options.center)
            for (var l = this.num.nav.length - 1; l >= 0 && (h += this.num.nav[l], this.dom.$oItems.eq(l).data("owl-item").page = d - 1, !(h >= s)); l--);
        ...
        if (i = i || 0, this.state.lazyContent) this.dom.$content = this.dom.$content.add(t(s)), this.updateItemState(!0);
        else {
            var e = this.fillItem(s);
            if (0 === this.dom.$oItems.length) this.dom.$stage.append(e);
            else {
                var o = this.dom.$oItems.eq(i); - 1 !== i ? o.before(e) : o.after(e)
            }
            this.refresh()
        }
        ...
        var s = this.dom.$items.eq(this.pos.prev),
            i = Math.abs(s.data("owl-item").width) * this.pos.prev,
            e = this.dom.$items.eq(this.pos.currentAbs),
            o = Math.abs(e.data("owl-item").width) * this.pos.currentAbs;
        if (this.pos.currentAbs === this.pos.prev) return !1;
        var n = o - i,
            a = this.options.animateIn,
            h = this.options.animateOut,
            r = this;
        removeStyles = function() {
            t(this).css({
                left: ""
            }).removeClass("animated owl-animated-out owl-animated-in").removeClass(a).removeClass(h), r.transitionEnd()
        }, h && s.css({
            left: n + "px"
        }).addClass("animated owl-animated-out " + h).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", removeStyles), a && e.addClass("animated owl-animated-in " + a).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", removeStyles)
